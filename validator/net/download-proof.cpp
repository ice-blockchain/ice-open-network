...
void DownloadProof::got_node_to_download(adnl::AdnlNodeIdShort node) {
  ...
  if (!is_key_block_) {
    query = create_serialize_tl_object<ion_api::ionNode_prepareBlockProof>(create_tl_block_id(block_id_),
                                                                           allow_partial_proof_);
  } else {
    query = create_serialize_tl_object<ion_api::ionNode_prepareKeyBlockProof>(create_tl_block_id(block_id_),
                                                                              allow_partial_proof_);
  }
  ...
  if (client_.empty()) {
    ...
    td::actor::send_closure(client_, &adnl::AdnlExtClient::send_query, "get_prepare",
                            create_serialize_tl_object_suffix<ion_api::ionNode_query>(std::move(query)),
...
void DownloadProof::got_block_proof_description(td::BufferSlice proof_description) {
  ...
  auto F = fetch_tl_object<ion_api::ionNode_PreparedProof>(std::move(proof_description), true);
  if (F.is_error()) {
  ...
  ion_api::downcast_call(
      *F.move_as_ok().get(),
      td::overloaded(
          [&](ion_api::ionNode_preparedProof &obj) {
            ...
            td::BufferSlice query;
            if (!is_key_block_) {
              query = create_serialize_tl_object<ion_api::ionNode_downloadBlockProof>(create_tl_block_id(block_id_));
            } else {
              query = create_serialize_tl_object<ion_api::ionNode_downloadKeyBlockProof>(create_tl_block_id(block_id_));
            }
            if (client_.empty()) {
          ...
          [&](ion_api::ionNode_preparedProofLink &obj) {
            if (!allow_partial_proof_) {
            ...
            if (!is_key_block_) {
              query =
                  create_serialize_tl_object<ion_api::ionNode_downloadBlockProofLink>(create_tl_block_id(block_id_));
            } else {
              query =
                  create_serialize_tl_object<ion_api::ionNode_downloadKeyBlockProofLink>(create_tl_block_id(block_id_));
            }
            if (client_.empty()) {
              td::actor::send_closure(overlays_, &overlay::Overlays::send_query_via, download_from_, local_id_,
          ...
          [&](ion_api::ionNode_preparedProofEmpty &obj) {
