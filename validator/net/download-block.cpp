...
void DownloadBlock::got_node_to_download(adnl::AdnlNodeIdShort node) {
  ...
  auto q = create_serialize_tl_object<ion_api::ionNode_prepareBlockProof>(create_tl_block_id(block_id_),
      ...
      *F.move_as_ok().get(),
      td::overloaded(
          [&](ion_api::ionNode_preparedProof &obj) {
            ...
            auto q = create_serialize_tl_object<ion_api::ionNode_downloadBlockProof>(create_tl_block_id(block_id_));
            if (client_.empty()) {
          ...
          [&](ion_api::ionNode_preparedProofLink &obj) {
            ...
            auto q = create_serialize_tl_object<ion_api::ionNode_downloadBlockProofLink>(create_tl_block_id(block_id_));
            if (client_.empty()) {
              td::actor::send_closure(overlays_, &overlay::Overlays::send_query_via, download_from_, local_id_,
...
void DownloadBlock::got_block_handle_2(BlockHandle handle) {
  ...
  if (handle_->received()) {
    ...
    auto q = create_serialize_tl_object<ion_api::ionNode_prepareBlock>(create_tl_block_id(block_id_));
    if (client_.empty()) {
      td::actor::send_closure(overlays_, &overlay::Overlays::send_query, download_from_, local_id_, overlay_id_,
...
void DownloadBlock::got_block_data_description(td::BufferSlice data_description) {
  ...
  auto F = fetch_tl_object<ion_api::ionNode_Prepared>(std::move(data_description), true);
  if (F.is_error()) {
    abort_query(F.move_as_error());
    return;
  }
  ...
  ion_api::downcast_call(
      *f.get(),
      td::overloaded(
          [&, self = this](ion_api::ionNode_prepared &val) {
            auto P = td::PromiseCreator::lambda([SelfId = actor_id(self)](td::Result<td::BufferSlice> R) {
              if (R.is_error()) {
                td::actor::send_closure(SelfId, &DownloadBlock::abort_query, R.move_as_error());
              } else {
            ...
            auto q = create_serialize_tl_object<ion_api::ionNode_downloadBlock>(create_tl_block_id(block_id_));
            if (client_.empty()) {
              td::actor::send_closure(overlays_, &overlay::Overlays::send_query_via, download_from_, local_id_,
