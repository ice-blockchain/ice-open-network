...
void GetNextKeyBlocks::got_node_to_download(adnl::AdnlNodeIdShort node) {
  ...
  auto query = create_serialize_tl_object<ion_api::ionNode_getNextKeyBlockIds>(create_tl_block_id(block_id_), limit_);
  if (client_.empty()) {
...
void GetNextKeyBlocks::got_result(td::BufferSlice data) {
  auto F = fetch_tl_object<ion_api::ionNode_keyBlocks>(std::move(data), true);
  if (F.is_error()) {
