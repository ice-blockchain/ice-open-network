...
void register_ion_gas_ops(OpcodeTable& cp0) {
  using namespace std::placeholders;
}
...
void register_ion_config_ops(OpcodeTable& cp0) {
  using namespace std::placeholders;
...
void register_ion_crypto_ops(OpcodeTable& cp0) {
  using namespace std::placeholders;
...
void register_ion_misc_ops(OpcodeTable& cp0) {
  using namespace std::placeholders;
...
void register_ion_currency_address_ops(OpcodeTable& cp0) {
  using namespace std::placeholders;
...
void register_ion_message_ops(OpcodeTable& cp0) {
  using namespace std::placeholders;
...
void register_ion_ops(OpcodeTable& cp0) {
  register_basic_gas_ops(cp0);
  register_ion_gas_ops(cp0);
  register_prng_ops(cp0);
  register_ion_config_ops(cp0);
  register_ion_crypto_ops(cp0);
  register_ion_misc_ops(cp0);
  register_ion_currency_address_ops(cp0);
  register_ion_message_ops(cp0);
}
...
bool parse_std_addr_q(CellSlice cs, ion::WorkchainId& res_wc, ion::StdSmcAddress& res_addr, bool quiet) {
...
std::pair<ion::WorkchainId, ion::StdSmcAddress> parse_std_addr(CellSlice cs) {
  std::pair<ion::WorkchainId, ion::StdSmcAddress> res;
