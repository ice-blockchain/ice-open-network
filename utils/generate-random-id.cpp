...
int main(int argc, char *argv[]) {
  ion::PrivateKey pk;
  td::optional<ion::adnl::AdnlAddressList> addr_list;
  ...
  p.add_checked_option('k', "key", "path to private key to import", [&](td::Slice key) {
    ...
    TRY_RESULT_PREFIX_ASSIGN(pk, ion::PrivateKey::import(data.as_slice()), "failed to import private key: ");
    return td::Status::OK();
  });
  p.add_checked_option('a', "addr-list", "addr list to sign", [&](td::Slice key) {
    ...
    ion::tl_object_ptr<ion::ion_api::adnl_addressList> addr_list_tl;
    TRY_STATUS_PREFIX(td::from_json(addr_list_tl, std::move(as_json_value)), "bad addr list TL: ");
    TRY_RESULT_PREFIX_ASSIGN(addr_list, ion::adnl::AdnlAddressList::create(addr_list_tl), "bad addr list: ");
    return td::Status::OK();
  });
  ...
  if (pk.empty()) {
    pk = ion::privkeys::Ed25519::random();
  }
  ...
  if (mode == "id") {
    ...
    std::cout << v << std::endl;
    v = td::json_encode<std::string>(td::ToJson(ion::adnl::AdnlNodeIdShort{short_key}.tl()));
    std::cout << v << std::endl;
  } else if (mode == "adnl") {
    if (!addr_list) {
    ...
    auto x = ion::create_tl_object<ion::ion_api::adnl_node>(pub_key.tl(), addr_list.value().tl());
    auto e = pk.create_decryptor().move_as_ok();
    auto r = e->sign(ion::serialize_tl_object(x, true).as_slice()).move_as_ok();
    ...
    td::BufferSlice to_sign = ion::serialize_tl_object(
        ion::dht::DhtNode{ion::adnl::AdnlNodeIdFull{pub_key}, addr_list.value(), -1, network_id, td::BufferSlice{}}
            .tl(),
        true);
    auto e = pk.create_decryptor().move_as_ok();
    auto signature = e->sign(to_sign.as_slice()).move_as_ok();
    auto node =
        ion::dht::DhtNode{ion::adnl::AdnlNodeIdFull{pub_key}, addr_list.value(), -1, network_id, std::move(signature)};
    ...
    std::cout << name << " " << ion::adnl::AdnlNodeIdShort{n}.serialize() << std::endl;
  } else {
